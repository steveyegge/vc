...
// Mission represents a high-level goal that gets broken down into child epics and tasks
// Missions are epics with subtype='mission' that can have regular child epics
// Dependencies establish parent-child relationships (simpler than phase ordering)
type Mission struct {
...
type MissionPlanner interface {
	...
	// RefinePhase generates detailed task breakdown for a specific planned epic
	// This is called when a child epic is ready to execute and needs granular tasks
	RefinePhase(ctx context.Context, plannedEpic *PlannedPhase, missionCtx *PlanningContext) ([]PlannedTask, error)

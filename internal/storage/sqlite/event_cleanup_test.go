...
func TestCleanupEventsByAge(t *testing.T) {
	...
	testEvents := []*events.AgentEvent{
		...
		// Old critical events (should be kept with longer retention - within 90 days)
		{ID: "evt-5", Type: "error", Timestamp: now.AddDate(0, 0, -80), IssueID: "test-1", ExecutorID: "exec-1", AgentID: "agent-1", Severity: "error", Message: "old error", Data: make(map[string]interface{})},
		{ID: "evt-6", Type: "error", Timestamp: now.AddDate(0, 0, -85), IssueID: "test-1", ExecutorID: "exec-1", AgentID: "agent-1", Severity: "critical", Message: "old critical", Data: make(map[string]interface{})},

		// Very old critical events (should be deleted - older than 90 days)
	...
	// Should delete 2 old regular events + 1 very old critical event = 3 total
	if deleted != 3 {
		t.Errorf("CleanupEventsByAge deleted %d events, want 3", deleted)
	}
	...
	if len(allEvents) != 4 {
		t.Errorf("got %d remaining events, want 4 (evt-3, evt-4, evt-5, evt-6)", len(allEvents))
	}

	// Verify the deleted events are gone
	deletedIDs := map[string]bool{"evt-1": true, "evt-2": true, "evt-7": true}
	for _, event := range allEvents {
		if deletedIDs[event.ID] {
...
func TestGetEventCounts(t *testing.T) {
	...
	// Create test issues
	createTestIssue(t, storage, "issue-1")
	createTestIssue(t, storage, "issue-2")
	createTestIssue(t, storage, "issue-3")
...
func TestVacuumDatabase(t *testing.T) {
	...
	// Create test issue for adding data
	createTestIssue(t, storage, "issue-1")
...
func TestCleanupEventsByAge_ContextCancellation(t *testing.T) {
	...
	if ctx.Err() == nil || (err != context.Canceled && !isContextCanceledError(err)) {
		t.Errorf("Expected context cancellation error, got: %v", err)
	}
}

// isContextCanceledError checks if an error is or wraps context.Canceled
func isContextCanceledError(err error) bool {
	...
	return err.Error() == "context canceled" ||
	       err.Error() == "failed to delete old regular events: context canceled" ||
	       err.Error() == "failed to delete old critical events: context canceled"
}
